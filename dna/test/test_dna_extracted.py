...
def test_read_gene_ids_from_file():
    filepath = os.path.join(os.path.dirname(__file__), "gene_lists/homo_sapiens_genes.txt")
...
def test_check_extracted_components():
    ...
    for species in gene_ids_filelist:
        ...
        extracted_dna_filepath = os.path.join(output_filepath, f"ensembl_data_{species_name}.csv")
        ground_truth_dna_filepath = os.path.join(os.path.dirname(__file__), f"ground_truth_components/true_{species_name}_dna.csv")
